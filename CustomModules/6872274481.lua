...
local ReducePackets = {["Enabled"] = false} -- pistonware modification
...
local bedwarsStore = {
	...
	whitelist = {
		chatStrings1 = {helloimusinginhaler = "vape", ["###################"] = "vape"}, -- pistonware modification
...
local function vapeGithubRequest(scripturl)
	if not isfile("vape/"..scripturl) then
		local suc, res = pcall(function() return game:HttpGet("https://raw.githubusercontent.com/supercellgamer/VapeV4ForRoblox/"..readfile("vape/commithash.txt").."/"..scripturl, true) end)
		assert(suc, res)
...
local function AllNearPosition(distance, amount, sortfunction, prediction, overridepos, ignore) -- pistonware modification
    ...
    if entityLibrary.isAlive then
		...
		for i, v in next, (entityLibrary.entityList) do
            ...
            if isVulnerable(v) then
				...
				if overridepos then mag = (overridepos - playerPosition).magnitude end -- pistonware modification
...
runFunction(function()
	...
	bedwars = {
		...
		ResetController = KnitClient.Controllers.ResetController, -- pistonware modification
	...
	bedwars.TweenDefault = require(replicatedStorageService.rbxts_include.node_modules["@rbxts"].tween).default -- pistonware modification
	...
	for i, v in next, ({"MatchEndEvent", "EntityDeathEvent", "EntityDamageEvent", "BedwarsBedBreak", "BalloonPopped", "AngelProgress", "StatefulEntityPlayerKnockbackRequest", "ScytheApplyState"}) do -- pistonware modification 
                ...
                if humrootpart and hum and head then
                    ...
                    if localcheck then
						...
						table.insert(newent.Connections, hum.AnimationPlayed:Connect(function(state) 
							...
							if animnum then
								local cached = entityLibrary.animationCache[state.Animation.AnimationId] -- pistonware modification (piston you have a self ddos or some shi)
								if not cached then 
...
local oldcloneroot -- pistonware modification
				...
				RunLoops:BindToHeartbeat("InfiniteFly", function(delta) 
					...
					if entityLibrary.isAlive then
						if isnetworkowner(oldcloneroot) then
							if _VapeApi.NoMove then return end -- pistonware modification
							local playerMass = (entityLibrary.character.HumanoidRootPart:GetMass() - 1.4) * (delta * 100)
...
runFunction(function()
    ...
    local killauracustomx = {Value = 0}
    local killauracustomy = {Value = 0}
    local killauracustomz = {Value = 0}
	...
	local anims = {
		...
		AstralOld = { -- pistonware modification
		...
		Sigma2 = { -- pistonware modification
		...
		Cockless = { -- pistonware modification
	...
	local meteorAnimations = {
		...
		["Exhibition2"] = {
		 ...
		 ["Old Extend"] = {
				{CFrame = CFrame.new(0.69, 0, 0.6) * CFrame.Angles(math.rad(-60), math.rad(30), math.rad(-40)), Time = 0.1},
				{CFrame = CFrame.new(0.99, -.2, 0.3) * CFrame.Angles(math.rad(-70), math.rad(10), math.rad(-20)), Time = 0.2},
				{CFrame = CFrame.new(1.49, -.2, 0.9) * CFrame.Angles(math.rad(-80), math.rad(0), math.rad(-20)), Time = 0.01},
    ...
    Killaura = GuiLibrary.ObjectsThatCanBeSaved.BlatantWindow.Api.CreateOptionsButton({
        ...
        Function = function(callback)
            if callback then
									...
									if killauraplaying == false then
										...
										for i,v in next, (anims[killauraanimmethod.Value]) do 
											...
											if not oldNearPlayer and killauraanimationtween.Enabled then
												gameCamera.Viewmodel.RightHand.RightWrist.C0 = originalArmC0 * v.CFrame + Vector3.new(killauracustomx.Value / 10, killauracustomy.Value / 10, killauracustomz.Value / 10)
												continue
											end
											killauracurrentanim = tweenService:Create(gameCamera.Viewmodel.RightHand.RightWrist, TweenInfo.new(v.Time), {C0 = originalArmC0 * v.CFrame + Vector3.new(killauracustomx.Value / 10, killauracustomy.Value / 10, killauracustomz.Value / 10)})
                ...
                task.spawn(function()
					repeat
						task.wait(ReducePackets.Enabled and ReduceKillAura.Enabled and 0.04 or 0) -- pistonware modification
    ...
    killauraanimation = Killaura.CreateToggle({
        ...
        Function = function(callback)
			...
			if killauracustomx.Object then killauracustomx.Object.Visible = callback end
			if killauracustomy.Object then killauracustomy.Object.Visible = callback end
			if killauracustomz.Object then killauracustomz.Object.Visible = callback end
		end,
	...
	killauracustomx = Killaura.CreateSlider({
        Name = "Custom X",
        Min = -30,
        Max = 30,
		Double = 10,
	...
	killauracustomx.Object.Visible = false
	killauracustomy = Killaura.CreateSlider({
        Name = "Custom Y",
        Min = -30,
        Max = 30,
		Double = 10,
	...
	killauracustomy.Object.Visible = false
	killauracustomz = Killaura.CreateSlider({
        Name = "Custom Z",
        Min = -30,
        Max = 30,
		Double = 10,
	...
	killauracustomz.Object.Visible = false
	...
	AutoBuy = GuiLibrary.ObjectsThatCanBeSaved.UtilityWindow.Api.CreateOptionsButton({
		...
		Function = function(callback)
			if callback then 
				...
				task.spawn(function()
					repeat
						task.wait(ReducePackets.Enabled and ReduceAutoBuy.Enabled and 0.2 or 0) -- pistonware modification
	...
	AutoBank = GuiLibrary.ObjectsThatCanBeSaved.UtilityWindow.Api.CreateOptionsButton({
		...
		Function = function(callback)
			if callback then
				...
				task.spawn(function()
					repeat
						task.wait(ReducePackets.Enabled and ReduceAutoBank.Enabled and 0.3 or 0) -- pistonware modification
			...
			if callback then
				task.spawn(function()
					...
					if (not ChestStealerSkywars.Enabled) or bedwarsStore.queueType:find("skywars") then
						repeat
							task.wait(ReducePackets.Enabled and ReduceChestStealer.Enabled and 0.3 or 0.1) -- pistonware modification
							if entityLibrary.isAlive then
	...
	task.spawn(function()
		pcall(function()
			...
			if not isfile("vape/Profiles/bedwarsdata.txt") then 
				local commit = "main"
				for i,v in next, (game:HttpGet("https://github.com/supercellgamer/VapeV4ForRoblox"):split("\n")) do 
				...
				writefile("vape/Profiles/bedwarsdata.txt", game:HttpGet("https://raw.githubusercontent.com/supercellgamer/VapeV4ForRoblox/"..commit.."/CustomModules/bedwarsdata", true))
			end
			...
			repeat
				...
				local newdata = game:HttpGet("https://raw.githubusercontent.com/supercellgamer/VapeV4ForRoblox/"..commit.."/CustomModules/bedwarsdata", true)
...
do -- so your don't hit the local limit w/ ur custom
	...
	runFunction(function()
		...
		InfiniteJump = GuiLibrary.ObjectsThatCanBeSaved.BlatantWindow.Api.CreateOptionsButton({
			Name = "InfiniteJump",
			HoverText = "Jump without touching ground (just like how piston got charged without touching those kids)",
			Function = function(callback)
				if callback then
					local held = false
	...
	runFunction(function()
		...
		local otherfakeplayers = {Name = "Scrxpted", UserId = "1"}
		...
		NameHider = GuiLibrary.ObjectsThatCanBeSaved.UtilityWindow.Api.CreateOptionsButton({
			...
			Function = function(callback)
				if callback then
					...
					singleWarning("Pistonware", "Join a new match to see names normally", 3) -- Not caching names and userids for disabling due to memory overloading + shit preformance
				end
			end
		})
	end)
			...
			Function = function(callback)
				if callback then
					if localMouse then
						...
						localMouse.Icon = "rbxassetid://14057134440"
	...
	runFunction(function() -- hello skid      hi piston, it's scrxpted
	...
	runFunction(function() -- will make this better when vape rewrite drops    sureeee
		...
		FloraIsFunny = GuiLibrary.ObjectsThatCanBeSaved.UtilityWindow.Api.CreateOptionsButton({
			...
			Function = function(callback)
				if callback then
					if bedwarsStore.equippedKit == "queen_bee" then
						if entityLibrary.isAlive then
							if entityLibrary.character.Humanoid.FloorMaterial ~= Enum.Material.Air then
								entityLibrary.character.Humanoid:ChangeState(Enum.HumanoidStateType.Jumping)
								local s = await(function() return entityLibrary.isAlive and entityLibrary.character.Humanoid.FloorMaterial == Enum.Material.Air end, 1)
								if s then
									useAbility("QUEEN_BEE_GLIDE")
								end
							else
								useAbility("QUEEN_BEE_GLIDE")
							end
						...
						singleWarning("FloatDisabler", "use the right kit lol", 5)
					end
					FloraIsFunny.ToggleButton(false)
				end
			end,
			HoverText = "Requires the Flora kit"
		})
	end)
	...
	runFunction(function()
		...
		Disabler = GuiLibrary.ObjectsThatCanBeSaved.UtilityWindow.Api.CreateOptionsButton({
				...
				if callback then 
					task.spawn(function()
						if GuiLibrary.ObjectsThatCanBeSaved.HeadlessOptionsButton.Api.Enabled then 
							GuiLibrary.ObjectsThatCanBeSaved.HeadlessOptionsButton.Api.ToggleButton(false)
							singleWarning("Disabler", "Disabled Headless to prevent bugs", 5)
						end
						repeat
							task.wait(ReducePackets.Enabled and ReduceDisabler.Enabled and 0.1 or 0.06) -- pistonware modification
							if (bedwarsStore.attackReachUpdate + 1) < tick() then
								local item = getItemNear("scythe")
								if entityLibrary.isAlive and item and lplr.Character.HandInvItem.Value == item.tool and bedwars.CombatController then
									bedwars.ClientHandler:Get("ScytheDash"):SendToServer({direction = entityLibrary.character.HumanoidRootPart.CFrame.lookVector})
									if entityLibrary.character.Head.Transparency ~= 0 and entityLibrary.character.Humanoid.FloorMaterial ~= Enum.Material.Air then
										bedwarsStore.scythe = tick() + 1
									end
								end
							end
			...
			HoverText = "I send packet and it work"
		})
	end)
	...
	runFunction(function()
		...
		--[=[
			Detections
			Chat A [detects chat messages with "<client>ware", "<client> on top", "piston", " | <client>" and "<client>v4" 
		...
		--[=[local _ProjectilesA = AntiCheat:Define("Projectiles", "A", function()local protectileHits = {}
			...
			table.insert(_ProjectilesA.Connections, vapeEvents.EntityDamageEvent.Event:Connect(function(damageTable)
				...
				local victim = playersService:GetPlayerFromCharacter(damageTable.entityInstance) -- piston knows all about this
				if player == lplr then return end
				if player then
		...
		local _SimulationC; _SimulationC = AntiCheat:Define("Simulation", "C (BETA)", function()
			...
			table.insert(_SimulationC.Connections, vapeEvents.EntityDamageEvent.Event:Connect(function(damageTable)
				if _SimulationMain.enabled() and (damageTable.damageType ~= 0 or damageTable.extra and damageTable.extra.chargeRatio ~= nil) and damageTable.entityInstance ~= lplr.Character then

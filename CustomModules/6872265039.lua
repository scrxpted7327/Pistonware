...
local function GetURL(scripturl)
	if shared.VapeDeveloper then
		...
		return game:HttpGet("https://raw.githubusercontent.com/supercellgamer/Pistonware/main/"..scripturl, true)
	end
end
...
local function getcustomassetfunc(path)
	if not betterisfile(path) then
		...
		local req = requestfunc({
			Url = "https://raw.githubusercontent.com/supercellgamer/Pistonware/main/"..path:gsub("vape/assets", "assets"),
			Method = "GET"
		})
...
local function getNametagString(plr)
	...
	if plrtag then
		for i, v in next, (plrtag) do 
			nametag = "<font color='#"..v.color:ToHex().."'>["..v.text.."]</font>"
		end
	end
	return nametag
end
...
local function renderNametag(plr)
	if WhitelistFunctions:GetWhitelist(plr) ~= "DEFAULT" or WhitelistFunctions.WhitelistTable.chattags[WhitelistFunctions:Hash(plr.Name..plr.UserId)] then
...
runcode(function()
	...
	local function disablestuff()
		...
		local function notlasso()
			for i,v in pairs(game:GetService("CollectionService"):GetWhitelistged("LassoHooked")) do 
	...
	local themefunctions = {
		Old = function()
			task.spawn(function()
				...
				task.spawn(function()
					for i,v in pairs(collectionservice:GetWhitelistged("block")) do
	...
	task.spawn(function()
		pcall(function()
			if not isfile("vape/Profiles/bedwarsdata.txt") then 
				local commit = "main"
				for i,v in pairs(game:HttpGet("https://github.com/supercellgamer/Pistonware"):split("\n")) do 
